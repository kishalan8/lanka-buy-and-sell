...
const EditBikeModel = ({ isOpen, onClose, refreshBikes, bike }) => {
  ...
  const [bikeImages, setBikeImages] = useState([]); // New images (File objects)
  const [retainedImages, setRetainedImages] = useState([]); // Old images to keep
  ...
  useEffect(() => {
    if (bike) {
      ...
      setBikeImages([]); // clear newly added files
      setRetainedImages(bike.images || []); // store old images
      setDocuments(
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Add retained images
      retainedImages.forEach(url => data.append("retainedImages[]", url));
      ...
      await axios.put(`http://localhost:5000/api/bikes/${bike._id}`, data, {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <motion.div className="bg-white rounded-xl max-w-3xl w-full p-6 relative overflow-auto max-h-[90vh]">
        {/* Tabs */}
