...
import { X, User, Briefcase, Mail, Phone, MapPin, ChevronDown } from 'lucide-react';
...
const EditLeadModal = ({ isOpen, onClose, onSave, lead }) => {
  ...
  // Populate form when lead prop changes
  useEffect(() => {
    if (lead) {
      setFormData({
        name: lead.name || '',
        ...
        status: lead.status || 'Initial Contact',
        assigned: lead.assigned || 'Sarah Johnson',
  ...
  const statusOptions = [
    'Initial Contact', 'Assessment', 'Documentation', 
    'Visa Processing', 'Offer Received', 'Completed', 'Rejected'
  ];

  const priorityOptions = ['High', 'Medium', 'Low'];
  const typeOptions = ['Customer', 'Agent'];
  const sourceOptions = ['Website', 'Referral', 'Advertisement', 'Event', 'Other'];
  const assignedOptions = ['Sarah Johnson', 'Mike Chen', 'David Wilson', 'Emma Davis'];
